    ...
    def init_xlsx(self, req):
        ...
        if req.get('as_path') is not None:
            ...
            for index,value in enumerate(['ATAC_seq','CTCF','H3K4me1', 'H3K4me3', 'H3K27ac', 'H3K27me3']):
                work_sheet[f"A{index+3}"] = value + "_m"
                work_sheet[f"B{index+3}"] = os.path.join('./lhjiang/nucleome_server/data', req['tissue'] + "_m_" + value + ".bigWig")
            for index,value in enumerate(['ATAC_seq','CTCF','H3K4me1', 'H3K4me3', 'H3K27ac', 'H3K27me3']):
                work_sheet[f"A{index+9}"] = value + "_p"
                work_sheet[f"B{index+9}"] = os.path.join('./lhjiang/nucleome_server/data', req['tissue'] + "_p_" + value + ".bigWig")
