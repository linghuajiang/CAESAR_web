...
temp_folder = '/nfs/turbo/umms-drjieliu/usr/lhjiang/nucleome_server/temp'
...
def find_1kb_region(position):
    ...
    center = (p11 + p22) / 2
    closest_center = int(round(center / 1000) * 1000)
...
def model_loop(weights='HFF6_loop_model_59.h5', first_layer=[96, 3],
    ...
    if verbose:
        m.summary()
    m.load_weights(os.path.join(path, weights))
    return m
...
def int_grad(hic, epigenetic, positions, steps=100):
    ...
    print(np.isnan(hics).any(), np.sum(hics))
    np.nan_to_num(hics, copy=False)
    ...
    with graph.as_default():
        with Session() as sess:
            model = model_fn()
            #######################################################################
            model_lp = model_loop()   # the path of the loop model
            ########################################################################

            grad = K.gradients(
                model.outputs[0][:, positions[0]:positions[1], positions[2]:positions[3]],
                model.inputs)
            print(model.outputs[0][:, positions[0]:positions[1], positions[2]:positions[3]], '------------')
            print(positions)
            # Tensor("strided_slice_1:0", shape=(?, 131, 0), dtype=float32) chr1:56824046-56850191,chr1:56983984-56989488 [495, 626, 1295, 1322]
            # Tensor("strided_slice_1:0", shape=(?, 95, 23), dtype=float32) chr1:56775885-56794805,chr1:56891101-56895572
            # Tensor("strided_slice_1:0", shape=(?, 259, 41), dtype=float32)
            ...
            for s in range(0, steps, 20):
                ...
                print(np.isnan(_grad).any(), np.sum(_grad))
                np.nan_to_num(_grad, copy=False)
                #######################################################################
                _grad_lp = sess.run(
                    grad_lp,
                    feed_dict={
                ...
                print(np.isnan(_grad_lp).any(), np.sum(_grad_lp))
                np.nan_to_num(_grad_lp, copy=False)
                #######################################################################

                grad_res[s:s+20, :, :] = _grad + _grad_lp * 0.4 ##################################
                np.nan_to_num(grad_res, copy=False)
            # K.clear_session()  ####################################
...
def save_bigBed(attributions, signals, ch, start_pos, req):
    port = req['port']
    #attributions = np.nan_to_num(attributions, copy=False)
    #print(np.isnan(attributions).any())
