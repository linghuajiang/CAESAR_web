from keras.layers import Input, Dropout, Concatenate, Permute, Conv1D, Add, BatchNormalization
...
source_path = hirespy.app.config['SRC_FOLDER']
...
def load_all_data(ch, start_pos, signals, main):
    hic = load_hic_data(ch, start_pos, main)
    epi = load_epigenetic_data([ch], signals)
    epi = epi[ch][start_pos // 200 - 7: start_pos // 200 + 1257, :]
    return hic, epi


def load_hic_data(ch, start_pos, main):
    os.system(f'touch {source_path}/temp/temp.txt')
    os.system(f'java -jar {path}/juicer_tools.jar dump observed NONE {main} \
{ch}:{start_pos}:{start_pos+250000} {ch}:{start_pos}:{start_pos+250000} BP 1000 {source_path}/temp/temp.txt')
    mat = np.zeros((250, 250))
    # print(f'java -jar {path}/juicer_tools.jar dump observed NONE /var/www/data/main/H1-hESC_full.hic \
# {ch}:{start_pos}:{start_pos+250000} {ch}:{start_pos}:{start_pos+250000} BP 1000 {source_path}/temp/temp.txt')
    for line in open(f'{source_path}/temp/temp.txt'):
    ...
    mat = f(new_co, new_co)
    os.system(f'rm {source_path}/temp/temp.txt')
    return np.log(mat + 1)


def load_epigenetic_data(chromosomes, signals):
...
def model_fn(model_weights='v23_temp_model_25.h5',
             first_layer=[96, 15], gcn_layers=[128, 128],
             conv_layer_filters=[128], conv_layer_windows=[15],
             nBins=1250, nMarks=8, lr=0.0001, verbose=False):
    """
    Build the GCN model
...
def int_grad(model, hic, epigenetic, positions, steps=100):
    """
    :param chrom:
    :param start_pos:
    ...
    sess = K.get_session()
    grad_res = sess.run(grad, feed_dict={model.inputs[0]: hics, model.inputs[1]: functionals})
    m = np.sum(grad_res[1], axis=0) / 100
    return m[7:-7, :]  # Remove the 14kb padding region


def save_bigBed(attributions, signals, ch, start_pos, output):
    original_attribution = deepcopy(attributions)
    # Set threshold:
    threshold = np.quantile(attributions, 0.9)
    ...
    for i, signal in enumerate(signals):
        f = open(f'{source_path}/temp/{signal}_temp.bed', 'w+')
        ...
        # Convert into bigBed
        os.system(f'sort -k1,1 -k2,2n {source_path}/temp/{signal}_temp.bed > {source_path}/temp/{signal}_sorted_temp.bed')
        ...
        os.system(f'{path}/bedGraphToBigWig {source_path}/temp/{signal}_sorted_temp.bed {path}/hg38.chrom.sizes {source_path}/output_bigWig/{signal}_temp.bigWig')
        os.system(f'rm {source_path}/temp/{signal}_temp.bed')
        #os.system(f'rm {source_path}/temp/{signal}_sorted_temp.bed')
        os.system(f'cp {source_path}/output_bigWig/{signal}_temp.bigWig {output}/{signal}.bigWig')
