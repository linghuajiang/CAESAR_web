...
def attribution(req, signals, cell_type='mESC', verbose=1):
    ...
    coordinate = req['region']
    port = req['port']

    # database progress
    update(40, "Identify the chosen region...", port)
    ...
    # database progress
    update(45, "Loading data for calculation...", port)

    if verbose:
        print(' Loading data for calculation...')
    # Step 3: Load data for the chosen region
    hic, epi = load_all_data(ch, start_pos, signals, req)

    # database progress
    update(50, "Calculating attributions...(This may take a while)", port)
    ...
    # database progress
    update(70, "Saving outputs...", port)

    if verbose:
        print(' Saving outputs...')
    # Step 5: Save them into bed file and convert into bigBed file
    save_bigBed(attributions, signals, ch, start_pos, req)
    return position
